...
cd /usr/local/k8s/work
source /usr/local/k8s/bin/environment.sh
cfssl gencert -ca=/usr/local/k8s/work/ca.pem \
  -ca-key=/usr/local/k8s/work/ca-key.pem \
  -config=/usr/local/k8s/work/ca-config.json \
...
kubectl config set-cluster kubernetes \
  --certificate-authority=/usr/local/k8s/work/ca.pem \
  --embed-certs=true \
